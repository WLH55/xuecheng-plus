...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public MediaFiles addMediaFilesToDb(Long companyId, String fileMd5, UploadFileParamsDto uploadFileParamsDto, String bucket, String objectName) {
        ...
        //添加到待处理任务表
        currentProxy.addWaitingTask(mediaFiles);
    ...
    @Override
    public RestResponse uploadChunk(String fileMd5, int chunk, String localPath) {
        //得到分块文件的目录路径
        String chunkFileFolderPath = getChunkFileFolderPath(fileMd5);
        //得到分块文件的路径
        ...
        try {
            //将文件存储至minIO
            addMediaFilesToMinIO(localPath, mimeType,bucket_video,chunkFilePath);
            return RestResponse.success(true);
        } catch (Exception ex) {
            ex.printStackTrace();
            log.debug("上传分块文件:{},失败:{}",chunkFilePath,ex.getMessage());
        }
        return RestResponse.fail(false,"上传失败");
    }
